...
struct zip_t *zip_open(const char *zipname, int level, char mode)
{
    ...
    switch (mode)
    {
        ...
        case 'a':
            ...
            if (mode == 'a' && !mz_zip_writer_init_from_reader(&(zip->archive), zipname))
            {
...
int zip_entry_close(struct zip_t *zip)
{
    ...
    if (!mz_zip_writer_create_local_dir_header(pzip, zip->entry.header, entrylen, 0, zip->entry.uncomp_size,
    ...
    if (pzip->m_pWrite(pzip->m_pIO_opaque, zip->entry.header_offset, zip->entry.header, sizeof(zip->entry.header)) !=
    ...
    if (!mz_zip_writer_add_to_central_dir(pzip, zip->entry.name, entrylen, NULL, 0, "", 0, zip->entry.uncomp_size,
                                          zip->entry.comp_size, zip->entry.uncomp_crc32, zip->entry.method, 0, dos_time,
...
int zip_entry_extract(struct zip_t *zip,
                      size_t (*on_extract)(void *arg, unsigned long long offset, const void *buf, size_t bufsize),
                      void *arg)
{
    mz_zip_archive *pzip = NULL;
    mz_uint idx;
...
int zip_create(const char *zipname, const char *filenames[], size_t len)
{
    ...
    for (i = 0; i < len; ++i)
    {
        ...
        if (!mz_zip_writer_add_file(&zip_archive, base_name(name), name, "", 0, ZIP_DEFAULT_COMPRESSION_LEVEL,
                                    ext_attributes))
        {
        ...
        { // and has sym link attribute (0x80 is file, 0x40
          // is directory)
            ...
            if (info.m_uncomp_size > MAX_PATH ||
                !mz_zip_reader_extract_to_mem_no_alloc(&zip_archive, i, symlink_to, MAX_PATH, 0, NULL, 0))
            {
                goto out;
            }
