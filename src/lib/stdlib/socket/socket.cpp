...
extern "C"
{
    EXPORTED cube_native_var *tcp_udp_socket_create(cube_native_var *port, cube_native_var *address,
                                                    cube_native_var *udp, cube_native_var *broadcast,
                                                    cube_native_var *reusesock, cube_native_var *isServer,
                                                    cube_native_var *timeout)
    {
        TcpUdpSocket *socket = new TcpUdpSocket(AS_NATIVE_NUMBER(port), AS_NATIVE_STRING(address), AS_NATIVE_BOOL(udp),
    ...
    EXPORTED cube_native_var *tcp_udp_socket_send_to(cube_native_var *pointer, cube_native_var *address,
                                                     cube_native_var *data)
    {
        ...
        AS_NATIVE_NUMBER(ret) = socket->WriteTo((char *)AS_NATIVE_BYTES(data).bytes, AS_NATIVE_BYTES(data).length,
