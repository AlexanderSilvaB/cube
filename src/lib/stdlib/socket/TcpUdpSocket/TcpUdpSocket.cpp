...
TcpUdpSocket::TcpUdpSocket(int port, char *address, bool udp, bool broadcast, bool reusesock, bool isServer,
                           int timeout)
{
    connected = false;
    received[0] = '\0';
...
const char *TcpUdpSocket::getAddress(const char *name)
{
    ...
    else
    {
        struct hostent *hostp =
            gethostbyaddr((const char *)&outaddr.sin_addr.s_addr, sizeof(outaddr.sin_addr.s_addr), AF_INET);
        if (hostp == NULL)
            return 0;
